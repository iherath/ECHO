...
for config in "${TASK_CONFIGS[@]}"; do
    ...
    for RUN in $(seq 1 "$N"); do
        ...
        sbatch \
            --job-name="$JOB_NAME" \
            --export=ALL,TASK="$TASK",SEED="$RUN",RUN="$RUN",NUM_STEPS="$NUM_STEPS",NUM_LAYERS="$NUM_LAYERS",LR="$LR",MAX_EPOCHS="$MAX_EPOCHS",HYPER_HIDDEN="$HYPER_HIDDEN",WINDOW_SIZE="$WINDOW_SIZE" \
